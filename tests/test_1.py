...
class MockResponseGET:
    def __init__(self, url, params=None, random_sid=None, **kwargs):
        ...
        assert (
            "headers" in kwargs
        ), "Проверьте, что вы передали загаловки `headers` для запроса статуса домашней работы"
        assert (
            "Authorization" in kwargs["headers"]
...
class MockTelegramBot:
    ...
    def send_message(self, chat_id=None, text=None, **kwargs):
        assert (
            chat_id is not None
        ), "Проверьте, что вы передали chat_id= при отправки сообщения ботом Telegram"
        assert (
            text is not None
        ), "Проверьте, что вы передали text= при отправки сообщения ботом Telegram"
...
class TestHomework:
    ...
    def test_parse_homework_status(self, random_sid):
        ...
        assert result.startswith(
            f'У вас проверили работу "{random_sid}"'
